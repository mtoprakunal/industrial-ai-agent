...
**Not 7 — `DefaultSessionFactory` vs `Session.Create` ve ApplicationUri Eşleşmesi**
Güvenli bağlantıda en sinsi hata: istemci sertifikasının `SubjectAlternativeName`'inde gömülü ApplicationUri ile `ApplicationConfiguration.ApplicationUri` birbirini tutmazsa, sunucu el sıkışmayı `BadCertificateUriInvalid` ile reddeder. SDK sertifikayı ilk çalıştırmada `ApplicationUri`'den üretir; sonradan `ApplicationName` veya makine adı (`Dns.GetHostName()` URI'ye girer) değişirse eski sertifika geçersiz kalır. Belirti: "önce çalışıyordu, makineyi yeniden adlandırınca/VM klonlayınca bozuldu". Çözüm: `pki/own` klasörünü silip sertifikayı yeniden ürettirin veya URI'yi sabit bir değere pinleyin.

**Not 8 — `Subscription.PublishingInterval` Geri Yazılır (Revised)**
`new Subscription { PublishingInterval = 100 }` ayarlasanız bile, `CreateAsync` sonrası gerçek değer `subscription.CurrentPublishingInterval`'dadır ve CODESYS bunu kendi minimumuna (sık 50-250ms) yuvarlar. Aynı şekilde `KeepAliveCount` ve `LifetimeCount` revize edilir. `LifetimeCount`, `KeepAliveCount`'un en az 3 katı olmalıdır; aksi halde sunucu subscription'ı erken siler ve sessizce veri akışı durur. SDK uyarı vermez — `CurrentLifetimeCount`'u loglayın.

**Not 9 — `MonitoredItem.Notification` Event'i ThreadPool Thread'inde, Sıra Garantisiz**
.NET SDK publish callback'lerini bir `ThreadPool` thread'inde işler. `SequentialPublishing = false` (varsayılan) iken, ardışık iki PublishResponse FARKLI thread'lerde paralel işlenebilir; yüksek frekanslı bir tag'de eski değer yeni değerden SONRA UI'ye ulaşabilir (out-of-order). Sayaç/setpoint gibi sıralama kritik tag'lerde `SequentialPublishing = true` zorunludur. Bu, .NET'i Python asyncua'dan ayıran önemli bir davranıştır (asyncua tek loop'ta doğal sıralıdır).

**Not 10 — WinForms `Control.Invoke` vs WPF `Dispatcher` Farkı**
WPF'de `Application.Current.Dispatcher.BeginInvoke` kullanılır; WinForms'ta ise `control.BeginInvoke(...)` veya `SynchronizationContext` gerekir. Bir projede WPF kodu WinForms'a port edilirken `Dispatcher` bırakıldı; `Application.Current` WinForms'ta null döndüğü için NullReferenceException tüm subscription thread'ini sessizce öldürdü ve veri durdu (exception ThreadPool thread'inde yutuldu). Cross-thread marshaling kütüphanesi UI framework'üne göre seçilmelidir; ortak çözüm başlangıçta `SynchronizationContext.Current`'ı yakalayıp `Post()` ile kullanmaktır.

## Edge Case'ler ve Sistem Limitleri

| Sınır / Edge Case | Davranış | Pratik Etki |
|---|---|---|
| `Session.ReadValue` (senkron) UI thread'inde | Ağ RTT kadar UI bloke | WPF/WinForms donar; daima `ReadAsync` + `await` |
| `BadTooManyOperations` | Tek Read/Write'taki node sayısı `MaxNodesPerRead/Write` aşıldı | İstek tümden reddedilir; `OperationLimits`'e göre chunk'la |
| `OperationLimits` node'u okunmadan toplu işlem | Sunucu limitini bilmeden büyük batch | Önce `Server/ServerCapabilities/OperationLimits`'i okuyup chunk boyutu belirleyin |
| `MaxNotificationsPerPublish` çok düşük | Bir publish'e sığmayan bildirim sonraki publish'e taşar | Burst sırasında bildirim gecikir; değeri 1000+ tutun |
| `LifetimeCount < 3 × KeepAliveCount` | Sunucu subscription'ı timeout'tan siler | Veri sessizce durur; reconnect handler bile fark etmeyebilir |
| `KeepAliveInterval` (Session) çok uzun | Ağ kopması geç algılanır | Reconnect gecikir; HMI eski veriyi canlı sanır |
| Sertifika süresi doldu | `BadCertificateTimeInvalid` | Sistem saati yanlışsa (RTC pili bitik IPC) yeni kurulan sertifika "gelecekte" görünür ve reddedilir |
| `MaxByteStringLength`/`MaxMessageSize` aşımı | Büyük array/struct okumada `BadEncodingLimitsExceeded` | `TransportQuotas` artırılmalı (her iki uçta da) |
| MaxSessions (CODESYS varsayılan 10) | `BadTooManySessions` | Ghost session'lar runtime restart'a kadar slot tutar |
| `complexTypeSystem` yüklenmeden custom struct okuma | Değer `ExtensionObject` olarak ham byte döner | `ComplexTypeSystem.Load()` çağrılmazsa CODESYS STRUCT'ları decode edilemez |

**Sertifika ve sistem saati tuzağı:** Endüstriyel panel PC'lerde RTC pili bittiğinde saat 2000 yılına döner. SDK yeni sertifikayı "şu andan" başlatır; sunucu doğru saattedir ve sertifikayı "geçerlilik gelecekte" diye reddeder (`BadCertificateTimeInvalid`). Belirti: "yeni cihazda ilk kurulumda güvenli bağlantı kurulamıyor ama UaExpert ile None modunda çalışıyor". NTP senkronizasyonu güvenli OPC UA'nın ön koşuludur.

**Custom STRUCT/ENUM decode:** CODESYS'te tanımlı kullanıcı STRUCT'ları (örn. `ST_MotorData`) OPC UA'da custom DataType olarak yayımlanır. .NET SDK bunları otomatik decode etmez; `await session.LoadDataTypeSystem()` (veya `ComplexTypeSystem`) çağrılmazsa okunan değer `ExtensionObject` içinde ham binary kalır. Büyük struct ağaçlarında bu yükleme bağlantı kurulumunu birkaç saniye uzatır — başlangıçta bir kez yapın, her okumada değil.

## Optimizasyon

**1. Senkron API'yi UI thread'inden tamamen kaldır.** `ReadValue`, `Write`, `Browse` senkron versiyonları RTT boyunca çağıran thread'i bloke eder. WPF/WinForms'ta daima `ReadAsync`/`WriteAsync` + `await` kullanın; `.Result`/`.GetAwaiter().GetResult()` UI thread'inde deadlock üretir (SynchronizationContext tuzağı).

**2. `OperationLimits`'e göre dinamik chunk.** Bağlantı sonrası `Server/ServerCapabilities/OperationLimits/MaxNodesPerRead` (ve `...PerWrite`, `...PerMonitoredItemCreate`) node'larını okuyun. Toplu Read/Write/MonitoredItem-create işlemlerini bu sınırın altında chunk'layın. Sabit 1000 varsaymak SL lisanslı düşük-limitli sunucularda `BadTooManyOperations` üretir.

**3. Tek Subscription + DataChangeFilter (Deadband).** Tüm tag'leri tek Subscription altında toplayın. Analog tag'lere `DataChangeFilter` ile `DeadbandType.Absolute/Percent` uygulayarak gürültü kaynaklı publish'i kesin:
```csharp
var item = new MonitoredItem(sub.DefaultItem)
{
    StartNodeId = NodeId.Parse(tag),
    Filter = new DataChangeFilter
    {
        Trigger = DataChangeTrigger.StatusValue,
        DeadbandType = (uint)DeadbandType.Absolute,
        DeadbandValue = 0.5   // 0.5 birim altı değişimi bildirme
    }
};
```

**4. `DequeueValues()` ile burst'ü topluca işle.** `Notification` event'inde tek değer değil, `item.DequeueValues()` ile kuyruktaki TÜM birikmiş değerleri tek seferde alın. UI için yalnızca SON değer anlamlıdır; ara değerleri arşiv/trend'e yazıp UI'ye sadece sonuncuyu marshaling edin (Dispatcher çağrısını azaltır).

**5. Dispatcher çağrılarını coalesce et.** Her notification'da `Dispatcher.BeginInvoke` çağırmak UI mesaj kuyruğunu doldurur. Bunun yerine değerleri thread-safe bir tampona yazıp (ör. `ConcurrentDictionary`), `DispatcherTimer` (100ms) ile toplu olarak UI'ye basın. 200 tag × 500ms'de bu, saniyede 400 Dispatcher çağrısını ~10'a indirir.

**6. `MinSupportedSampleRate` ve publishing dengesi.** HMI için 250-1000ms PublishingInterval yeterlidir. Alarm/interlock tag'lerini ayrı, hızlı (100ms) bir Subscription'a; trend/gösterge tag'lerini yavaş (1000ms) Subscription'a koyun. Tek hızlı subscription'da yüzlerce yavaş tag taşımak hem sunucu CPU'sunu hem ağı yorar.

**7. Session/Channel yeniden kullanımı.** Her ekran geçişinde yeni Session açıp kapatmayın — handshake (channel + ActivateSession + sertifika doğrulama) pahalıdır. Tek uzun-ömürlü Session tutun, ekranlar yalnızca MonitoredItem ekleyip/çıkarsın (`Subscription.AddItem`/`RemoveItem` + `ApplyChanges`).

## Derin Teknik Detay

**`ISession`, `SessionReconnectHandler` ve `TransferSubscriptions` iç mekanizması.** OPC UA'da Session ve Secure Channel ayrı yaşam döngülerine sahiptir. `KeepAlive` event'i `Bad` durum bildirdiğinde `SessionReconnectHandler.BeginReconnect` devreye girer: önce mevcut Session nesnesini KORUYARAK yeni bir Secure Channel açmayı dener (`Session.Reconnect`). Bu, sunucudaki session'ı yeni kanala bağlar — subscription'lar sunucuda hâlâ canlıdır, veri kaybı olmaz. Eğer session sunucuda timeout'a düşmüşse (uzun kesinti), reconnect başarısız olur ve handler `TransferSubscriptionsOnReconnect = true` ise YENİ session açıp `TransferSubscriptions` servisiyle eski subscription ID'lerini yeni session'a taşımayı dener. CODESYS bu servisi her sürümde tam desteklemez; desteklemiyorsa SDK subscription'ları yeniden oluşturur (MonitoredItem'ler tekrar create edilir). `DeleteSubscriptionsOnClose = false` bu transferin ön koşuludur — true olursa session kapanırken subscription sunucuda silinir ve transfer edilecek bir şey kalmaz.

**`DefaultSessionFactory` neden eklendi, `Session.Create`'in sorunu neydi?** Eski statik `Session.Create()`, içeride bazı işlemleri senkron bloke edip `Task.Run` ile sarmalayan karma bir yapıdaydı; ASP.NET/WPF gibi `SynchronizationContext`'i olan ortamlarda deadlock'a ve thread havuzu açlığına yol açabiliyordu. `DefaultSessionFactory.CreateAsync`, tüm el sıkışmayı (GetEndpoints → OpenSecureChannel → CreateSession → ActivateSession) uçtan uca `async` zincir olarak yürütür ve `ISession` arayüzü döndürerek test edilebilirliği ve dependency injection'ı mümkün kılar. v1.5.378.145'te düzeltilen "Task.Delay sonsuz döngü / aşırı task spawn" hatası tam olarak bu async reconnect yolundaydı — eski sürümlerde reconnect döngüsü CPU'yu %100'e çıkarabiliyordu; üretim için bu sürüme yükseltme kritiktir.

**Publish mekanizması — istemci sunucuyu "yoklar".** OPC UA sunucu istemciye proaktif mesaj göndermez (firewall/NAT dostu olmak için). .NET SDK arka planda bir havuz dolusu `PublishRequest`'i sunucuya önceden gönderir; sunucu bildirecek veri oluştuğunda bekleyen request'lerden birine `PublishResponse` ile yanıtlar. SDK bu yanıtı bir ThreadPool thread'inde parse edip `MonitoredItem.Notification` event'ini tetikler ve havuza yeni PublishRequest ekler. Bu yüzden istemci uygulaması askıda kalır/ThreadPool açlığına girerse (örn. UI thread'inde `.Result` deadlock'u), yeni PublishRequest gönderilemez ve veri akışı durur — ama bağlantı "bağlı" görünmeye devam eder. Bu, .NET tarafında en zor teşhis edilen production sorunlarından biridir.

**`Variant` / `BuiltInType` ve CODESYS tip eşlemesi neden kritik?** OPC UA değerleri `Variant` içinde `BuiltInType` etiketiyle taşınır. CODESYS `REAL` → `Float` (Single, 32-bit), `LREAL` → `Double` (64-bit), `INT` → `Int16`, `DINT` → `Int32`, `WORD` → `UInt16`. `WriteAsync`'te `new Variant(75.0)` yazarsanız .NET bunu `Double` olarak işaretler; sunucu node'u `REAL` (Float) ise tip uyuşmazlığı `BadTypeMismatch` döndürür VEYA bazı sunucular sessizce reddeder. Bu yüzden yazarken hedef tipe açık cast şarttır (`(float)`, `(short)`). Okumada ise `DataValue.Value` `object` tipindedir; `Convert.ToDouble/ToInt32` ile dönüştürmek, sunucunun gönderdiği gerçek BuiltInType'tan bağımsız güvenli dönüşüm sağlar — bu yüzden ViewModel mapping'lerinde doğrudan cast yerine `Convert.*` tercih edilir.
