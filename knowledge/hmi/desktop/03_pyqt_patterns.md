...
**Not 7 — `Signal(object)` ile büyük veri kopyalama ve queued connection maliyeti:**
Queued connection'da sinyal argümanları Qt'nin meta-object sistemi üzerinden alıcı thread'in event kuyruğuna kopyalanır. Worker'dan her notification'da `Signal(dict)` ile 200 elemanlı sözlük göndermek, saniyede yüzlerce kopya + event kuyruğu basıncı yaratır. Bir projede 200 tag × 500ms subscription, her tag için ayrı `tag_updated.emit(tag, value)` çağırdığında GUI event kuyruğu doldu ve buton tıklamaları gecikmeye başladı. Çözüm: worker tarafında bir tampon biriktirip 100ms'de bir `batch_updated.emit(snapshot)` ile tek sinyal göndermek (coalescing). Sinyal frekansı, gerçek veri frekansından bağımsız olarak insan gözüne (10-15 Hz) sabitlenmelidir.

**Not 8 — `moveToThread` sonrası `__init__`'te oluşturulan QObject'ler eski thread'de kalır:**
Worker'ı `moveToThread(thread)` ile taşırsanız, worker'ın KENDİSİ taşınır; ancak `__init__` içinde oluşturulan child QObject'ler (örn. bir QTimer, bir QTcpSocket) parent ilişkisiyle taşınır AMA `__init__` worker henüz taşınmadan (GUI thread'inde) çalıştığı için, taşımadan sonra bunların thread affinity'si karışabilir. Güvenli pattern: ağ nesneleri ve timer'lar worker'ın `run()` slot'u İÇİNDE (yani hedef thread'de çalışırken) oluşturulmalıdır. Bu, "QObject: Cannot create children for a parent that is in a different thread" uyarısının en sık nedenidir.

**Not 9 — qasync ile asyncua: `closeEvent` async olamaz, pencere kapanışında asılı task:**
qasync kullanırken `closeEvent` senkrondur; içinde `await client.disconnect()` yapılamaz. `@asyncClose` dekoratörü veya `app.aboutToQuit` sinyaline bağlı bir async temizleme task'ı gerekir. Aksi halde uygulama kapanırken asyncua disconnect tamamlanmadan event loop durur, sunucuda ghost session kalır ve bir sonraki açılışta `Bad_TooManySessions` riski doğar. Sahada "uygulamayı birkaç kez açıp kapatınca artık bağlanmıyor" şikayetinin tipik nedeni budur.

**Not 10 — `asyncio.run_coroutine_threadsafe` dönüş Future'ı GUI thread'inde beklenirse donar:**
QThread içindeki ayrı asyncio loop'a iş göndermek için `asyncio.run_coroutine_threadsafe(coro, loop)` kullanılır; bu bir `concurrent.futures.Future` döndürür. GUI thread'inde `future.result()` çağırmak (sonucu beklemek) GUI'yi donduran klasik hatadır — coroutine bitene kadar bloke olur. Doğru yaklaşım: sonucu beklemeyin; coroutine işini bitirince Qt sinyali emit etsin, GUI o sinyalle güncellensin. Future yalnızca exception loglamak için `add_done_callback` ile izlenmelidir.

## Edge Case'ler ve Sistem Limitleri

| Sınır / Edge Case | Davranış | Pratik Etki |
|---|---|---|
| Worker thread'den doğrudan `widget.setText()` | Tanımsız davranış / crash (Qt thread affinity ihlali) | Daima Signal/Slot (queued) ile UI'ye geç |
| GUI thread'inde bloke I/O (PLC read) | UI donar, ~5sn sonra OS "Yanıt vermiyor" | Tüm I/O QThread/asyncio'ya taşınmalı |
| Queued connection event kuyruğu birikmesi | Sinyaller işlenemeyince RAM artar, gecikme büyür | Sinyal frekansını coalesce ile sınırla |
| `QTimer` worker thread'de, loop yok | Timer hiç tetiklenmez (sessizce) | Timer'ın yaşadığı thread'in event loop'u olmalı |
| `deleteLater()` çağrılan QObject'e erişim | Use-after-free / segfault | Sinyal bağlantılarını obje silinmeden kes |
| QLineSeries'e >10k nokta | Yeniden çizim donması (UI thread'de render) | PyQtGraph'a geç veya `replace()` + downsample |
| `Signal` emit'i loop bitmiş thread'e | Slot hiç çalışmaz, sessiz veri kaybı | Worker durdurulurken son sinyalleri flush et |
| PySide6 enum erişimi (`Qt.AlignCenter`) | PySide6 6.4+ scoped enum zorunlu olabilir | `Qt.AlignmentFlag.AlignCenter` formatı sürüm-güvenli |
| `asyncio.run` Qt loop içinde | İç içe loop hatası / RuntimeError | qasync veya ayrı thread loop kullan |
| PyInstaller `--onefile` + asyncua | Hidden import eksik → ImportError runtime'da | `hiddenimports` + `--onedir` tercih et |

**GIL ve "CPU-bound worker UI'yi yine de dondurabilir":** QThread veri okumayı GUI thread'inden ayırır, AMA Python GIL nedeniyle aynı anda yalnızca bir thread Python bytecode çalıştırır. Worker thread saf-Python ağır hesap yapıyorsa (büyük liste işleme, regex, JSON parse), GIL'i tutar ve GUI thread render/event işleme için GIL'i alamaz — UI takılır. I/O işlemleri (socket recv, dosya) GIL'i bırakır, bu yüzden saf veri okuma sorun değildir; sorun worker'daki CPU-bound post-processing'tir. Çözüm: ağır hesabı `numpy` (C içinde GIL bırakır) veya `ProcessPoolExecutor` (ayrı süreç) ile yapın.

**PySide6 vs PyQt6 enum/imza farkları:** Aynı Qt API'sini sarsalar da ince farklar vardır: PySide6 `@Slot`/`Signal` kullanır, PyQt6 `@pyqtSlot`/`pyqtSignal`. PySide6 6.4+ scoped enum'ları zorunlu kılabilir (`Qt.AlignmentFlag.AlignCenter`), eski `Qt.AlignCenter` bazı sürümlerde DeprecationWarning verir. Cross-binding kod yazmak için `qtpy` soyutlama katmanı kullanın; aksi halde binding'i sabitleyin.

## Optimizasyon

**1. Sinyal frekansını veri frekansından ayır (coalescing).** Subscription saniyede 400 değişim üretse bile UI'yi 10-15 Hz'den hızlı güncellemek gözle algılanmaz ve boşa CPU yakar. Worker'da gelen değerleri bir tampona yazın; bir QTimer (66-100ms) tamponun SON snapshot'ını alıp tek `batch_updated` sinyaliyle GUI'ye basın. Bu, hem queued connection maliyetini hem de yeniden boyama (repaint) sayısını dramatik düşürür.

**2. Model/View'da `dataChanged`'i dar tut.** `QAbstractTableModel.update_tag` içinde tüm tabloyu değil, yalnızca değişen satır/sütun aralığını `dataChanged.emit(topLeft, bottomRight, [roles])` ile bildirin. `[Qt.DisplayRole]` gibi rol listesi vermek, Qt'nin gereksiz rol sorgularını atlamasını sağlar. 1000 satırlık tag tablosunda tüm modeli reset etmek yerine tek hücre güncellemek 100x hızlıdır.

**3. Trend için PyQtGraph + `setData`, QLineSeries `clear()/append()` değil.** QtCharts'ta kayan pencere için `clear()` + döngüyle `append()` her noktada sahne grafiğini yeniden kurar; >1000 noktada donar. PyQtGraph'ta `curve.setData(x_array, y_array)` tek çağrıda numpy array'i çizer ve >5 Hz, >10k nokta senaryosunda akıcı kalır. QtCharts kullanılacaksa `replace(list)` ile toptan değişim `append` döngüsünden hızlıdır.

**4. Ekrandaki noktayı downsample et.** Bir trend widget'ı 1920px genişse, 100.000 veri noktasını çizmenin anlamı yoktur — ekran çözünürlüğünden fazla nokta görünmez. Çizimden önce veriyi piksel sayısına indirin (min/max decimation); historian'da tam çözünürlük kalsın, ekranda downsampled gösterilsin.

**5. QSS'i statik tut, her notification'da `setStyleSheet` çağırma.** `setStyleSheet()` o widget ve alt ağacı için stil yeniden derler/polish eder — pahalıdır. Alarm yanıp sönme gibi durumlarda iki hazır QSS string'i arasında geçiş yapın; mümkünse `setProperty` + `style().polish()` ile property-based selector kullanın ve string'i tekrar parse ettirmeyin.

**6. `update()` yerine `repaint()` çağırmayın.** `repaint()` senkron, anında yeniden çizer ve event loop'u keser; `update()` ise çizimi bir sonraki loop turuna erteler ve ardışık update'leri tek boyamaya birleştirir (coalescing). Gerçek zamanlı veri akışında daima `update()` kullanın.

**7. Worker'da `loop.run_in_executor` ile post-processing'i ayır.** asyncio loop'unda gelen veriyi parse/dönüştürme CPU işi loop'u bloke eder (ve dolaylı olarak diğer tag'lerin işlenmesini geciktirir). Ağır dönüşümü `loop.run_in_executor(None, transform, raw)` ile thread havuzuna verin; loop yalnızca I/O ve dispatch'le ilgilensin.

## Derin Teknik Detay

**Queued connection iç mekanizması — neden thread-safe?** Qt'de `connect()` türü `Qt.AutoConnection` ise, emit anında gönderici ve alıcının `thread()` affinity'si karşılaştırılır. Farklıysa Qt, sinyal argümanlarını `QMetaType` ile serileştirip (kopyalayıp) bir `QMetaCallEvent` olarak alıcının ait olduğu thread'in `QEventLoop` kuyruğuna `postEvent` eder. Slot, o thread bir sonraki `processEvents` turunda kuyruğu işlerken, ALICI thread'de çalışır. Böylece slot içindeki UI erişimi GUI thread'inde gerçekleşir — kilit gerekmez çünkü veri kopyalanmış ve teslim event loop'a devredilmiştir. Bu yüzden Signal/Slot, manuel mutex'ten daha güvenli ve idiomatic'tir; ama bedeli argüman kopyalama maliyetidir (büyük dict'lerde hissedilir). `Qt.DirectConnection` zorlanırsa slot emit eden thread'de çalışır — UI nesnelerine dokunursa crash olur.

**QThread nesnesi nerede yaşar — affinity'nin kökü.** Bir `QThread` nesnesi onu OLUŞTURAN thread'e (genelde GUI thread) aittir; `run()` ise YENİ thread'de çalışır. Bu ayrım kritiktir: QThread'i alt sınıflayıp slot eklerseniz, o slot QThread nesnesinin affinity'sinde — yani GUI thread'inde — çalışır, yeni thread'de değil. Worker-object deseni bu tuzağı aşar: `worker.moveToThread(thread)` worker'ın affinity'sini yeni thread'e çevirir; `thread.started` (yeni thread'de emit edilir) → `worker.run` (queued, ama affinity yeni thread olduğu için yeni thread'de çalışır) bağlantısı sayesinde worker'ın tüm slot'ları artık yeni thread'de yürür. Qt'nin "event-driven nesneler (QTimer, socket) tek thread'de kullanılmalı" kuralı da buradan gelir: bu nesneler `timerEvent`/I/O event'lerini affinity thread'lerinin event loop'undan alır.

**qasync nasıl çalışır — Qt event loop'u asyncio loop'una dönüştürmek.** asyncio bir `AbstractEventLoop` soyutlaması tanımlar; qasync bu arayüzü Qt'nin `QEventLoop`/`QSocketNotifier` üzerine implemente eder (`QEventLoop` sınıfı asyncio API'sini sağlar). asyncio'nun `call_soon`, `call_later`, socket okuma/yazma kayıtları, Qt'nin `QTimer` ve `QSocketNotifier` mekanizmalarına çevrilir. Sonuç: `app.exec()` yerine tek bir birleşik döngü hem Qt event'lerini hem asyncio coroutine'lerini sürer. `@asyncSlot()` dekoratörü, bir Qt sinyaline bağlanan normal slot'u, çağrıldığında coroutine'i bu birleşik loop'a `ensure_future` ile schedule eden bir köprüye dönüştürür. Avantaj: her şey TEK thread'de (GUI thread) çalışır — Signal/Slot marshaling derdi yoktur, çünkü asyncua çağrıları da GUI thread'inde await edilir ve GIL salınmaz çünkü I/O sırasında await noktalarında loop diğer işleri sürdürür. Dezavantaj: bir await'siz uzun CPU işi yine tüm loop'u (ve UI'yi) durdurur.

**qasync (tek loop) vs QThread+asyncio (iki loop) — hangisi ne zaman?** qasync modeli daha basittir ve marshaling kodu gerektirmez; tek geliştiricili, orta karmaşıklıkta HMI için idealdir. Ancak tüm asyncua işi GUI thread'inde await edildiği için, beklenmedik bir senkron-bloke (örn. asyncua olmayan eski bir sync kütüphane çağrısı) doğrudan UI'yi dondurur. QThread + ayrı asyncio loop modeli, OPC UA tarafını GUI'den tamamen izole eder: worker thread çökse/bloklansa bile GUI canlı kalır, "yeniden bağlanılıyor" overlay'i akıcı çalışır. Bedeli: `run_coroutine_threadsafe` + Signal marshaling boilerplate'i ve iki event loop'un yaşam döngüsünü senkronize etme zorunluluğu (kapanışta worker loop'u `call_soon_threadsafe(loop.stop)` ile düzgün durdurulmalı). Üretim, 7/24, çok-ekranlı endüstriyel HMI'larda izolasyon nedeniyle ikinci model sıklıkla tercih edilir; bu belgenin OPC UA worker örneği bilinçli olarak bu modeli kullanır.
